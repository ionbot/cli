...
const installDeps = (dirName) => {
  ...
  exec("yarn", { cwd: dirName }, (err, stdout, stderr) => {
    spinner.text = "Bootstraping packages...";
    const bootstrap = spawn("npx", ["lerna", "bootstrap"], { cwd: dirName });
    bootstrap.stdout.on("data", function (data) {
      console.log(data.toString());
    });
    bootstrap.stderr.on("data", function (data) {
      console.log(data.toString());
    });
    bootstrap.on("exit", function (code) {
      console.log("code", code);
      console.log("Bootstrap finished");
      spinner.text = "Building Ion\n\n";

      const build = spawn("yarn", ["build"], { cwd: dirName });
...
const DownloadIon = () => {
  ...
  axios({
    method: "get",
    url: "https://github.com/ionbot/ion-app/archive/refs/heads/main.zip",
    responseType: "stream",
  }).then((response) => {
